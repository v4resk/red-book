...
_The offline Golden gMSA computation (deriving any gMSA password from a stolen KDS root key) currently has no UNIX-native tool and must be run from Windows with GoldenGMSA.exe. Reading a live gMSA password over LDAP, by contrast, is possible from UNIX with tools like gMSADumper or NetExec once the account can be queried._
...
1. [dump some information relative to the gMSA account](/ad/persistence/goldengmsa#account-information-dump) to retrieve the password for
2. use those elements to [calculate the gMSA password](/ad/persistence/goldengmsa#password-calculation)
...
In order to use the password, its MD4 (i.e. NT) hash can be calculated, for [pass-the-hash](/ad/movement/ntlm/pth).
...
<ResourceCard href="https://github.com/Semperis/GoldenGMSA" title="GitHub - Semperis/GoldenGMSA: GolenGMSA tool for working with GMSA passwords" description="github.com" icon="/assets/favicons/github.com.png" />

<ResourceCard href="https://www.semperis.com/blog/golden-gmsa-attack" title="www.semperis.com" description="www.semperis.com" icon="/assets/favicons/www.semperis.com.png" />

<ResourceCard href="https://www.trustedsec.com/blog/splunk-spl-queries-for-detecting-gmsa-attacks/" title="www.trustedsec.com" description="www.trustedsec.com" />
