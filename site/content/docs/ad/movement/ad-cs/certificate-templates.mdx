...
Understanding certificate mapping is pretty useful to understand [ESC9](/ad/movement/ad-cs/certificate-templates#no-security-extension-esc9) and [ESC10](/ad/movement/ad-cs/certificate-templates#weak-certificate-mapping-esc10), [ESC14](/ad/movement/ad-cs/certificate-templates#esc14-weak-explicit-mapping).
...
![explicit\_mapping.webp](/assets/explicit_mapping.CfHEWvSQ.webp)
...
![CT\_FLAG.webp](/assets/CT_FLAG.Df9ZBgv6.webp)
...
Following [CVE-2022–26923](https://research.ifcr.dk/certifried-active-directory-domain-privilege-escalation-cve-2022-26923-9e098fe298f4) ([certifried.md](/ad/movement/ad-cs/certifried)) discovered by [Olivier Lyak](https://twitter.com/ly4k_), Microsoft has implemented a new security extension for the issued certificates, and two new registry keys to properly deal with certificate mapping.
...
If you get an "Object SID mismatch" error during the authentication, this could mean that Kerberos is enforcing [strong certificate mapping](/ad/movement/ad-cs/certificate-templates#weak-and-strong-mapping). This is the default behavior since february 2025 ([more information](https://support.microsoft.com/en-us/topic/kb5014754-certificate-based-authentication-changes-on-windows-domain-controllers-ad2c23b0-15d8-4340-a468-4d4f3b188f16#bkmk_certmap)). It is then possible to specify the security identifier (using the certipy's flag `-sid`) corresponding at the specified `subjectAltName`.
...
To understand this privilege escalation, it is recommended to know how certificate mapping is performed. It is presented in [this section](/ad/movement/ad-cs/certificate-templates#certificate-mapping).
...
First, the **user2**'s hash is needed. It can be retrieved via a [Shadow Credentials](/ad/movement/kerberos/shadow-credentials) attack, for example.
...
Now, authenticating with the obtained certificate will provide the **user3**'s NT hash during [UnPac the hash](/ad/movement/kerberos/unpac-the-hash). The domain must be specified since it is not present in the certificate.
...
First, the **user2**'s credentials are needed. It can be retrieved via a [Shadow Credentials](/ad/movement/kerberos/shadow-credentials) attack, for example. Here just the `msDs-KeyCredentialLink` modification part with [Whisker](https://github.com/eladshamir/Whisker):
...
Now, authenticating with the obtained certificate will provide the **user3**'s NT hash during [UnPac the hash](/ad/movement/kerberos/unpac-the-hash). This action can be realised with [Rubeus](https://github.com/GhostPack/Rubeus). The domain must be specified since it is not present in the certificate.
...
Now, authenticating with the obtained certificate will provide the **user3**'s NT hash with [UnPac the hash](/ad/movement/kerberos/unpac-the-hash). The domain must be specified since it is not present in the certificate.
...
Now, authentication with the obtained certificate will be performed through Schannel. The `-ldap-shell` option can be used to execute some LDAP requests and, for example, realised an [RBCD](/ad/movement/kerberos/delegations/rbcd) to fully compromised the domain controller.
...
Now, authentication with the obtained certificate will be performed through Schannel. It can be used to perform, for example, an [RBCD](/ad/movement/kerberos/delegations/rbcd) to fully compromised the domain controller.

</Tab>
</Tabs>

### Issuance policy with privileged group linked (ESC13) <a href="#esc13-issuance-policiy-with-privileged-group-linked" id="esc13-issuance-policiy-with-privileged-group-linked"></a>
...
The certificate can then be used with [Pass-the-Certificate](/ad/movement/kerberos/pass-the/pass-the-certificate) to obtain a TGT and authenticate as the controlled principal, but with its privileges added to those of the linked group.
...
This attack exploits configuration errors when explicit mapping is set up. To understand this ESC, it is essential to have a good command of [Certificate mapping](/ad/movement/ad-cs/certificate-templates#certificate-mapping).
...
Write rights to `altSecurityIdentities` can be obtained via the following [DACL](/ad/movement/dacl):
...
The certificate requested at the begining can then be used with [Pass-the-Certificate](/ad/movement/kerberos/pass-the/pass-the-certificate) to obtain a TGT and authenticate as the target.
...
The certificate can then be used with [Pass-the-Certificate](/ad/movement/kerberos/pass-the/pass-the-certificate) to obtain a TGT and authenticate as the target.
...
From Windows systems, with PowerShell the `dNSHostName` attribute of the victim must be overwritten to be equal to the target's (`$TARGET`).
...
Note, however, that specifying "Client Authentication" in _application policy_ will only allow **SChannel authentication and not PKINIT**. On the other hand, by specifying "Certificate Request Agent" (`1.3.6.1.4.1.311.20.2.1`), the certificate can be used as an [ESC3](/ad/movement/ad-cs/certificate-templates#certificate-agent-eku-esc3).

</Callout>

You can see the ESC15 as an [ESC2 ](/ad/movement/ad-cs/certificate-templates#any-purpose-eku-esc2)with more steps.
...
In this scenario, the attacker has `GenericWrite` permission over a victim account, which can be used to overwrite the victim's UPN. Using the victim's NT hash obtained via a [Shadow Credentials](/ad/movement/kerberos/shadow-credentials) attack, it's possible to request a certificate from the CA on the victim's behalf. Thanks to the UPN manipulation, a valid certificate can be obtained for any user in the domain.
...
**This chain is not exploitable under full enforcement (`StrongCertificateBindingEnforcement = 2`).** Per SpecterOps, under full enforcement the KDC requires a valid `szOID_NTDS_CA_SECURITY_EXT` SID extension matching the impersonated principal. A CA vulnerable to ESC16 globally disables that extension, so no valid SID can be embedded in issued certificates and the KDC rejects authentication. The ESC16 + ESC6 combination below only bypasses compatibility mode (`0` or `1`), not full enforcement.
...
* CA vulnerable to [ESC6](/ad/movement/ad-cs/certificate-authority#esc6-editf-attributesubjectaltname2)
...
<ResourceCard href="https://www.thehacker.recipes/ad/movement/adcs/certificate-templates" title="Certificate templates | The Hacker Recipes" description="www.thehacker.recipes" icon="/assets/favicons/www.thehacker.recipes.png" />
...
<ResourceCard href="https://research.ifcr.dk/certipy-4-0-esc9-esc10-bloodhound-gui-new-authentication-and-request-methods-and-more-7237d88061f7" title="research.ifcr.dk" description="research.ifcr.dk" icon="/assets/favicons/research.ifcr.dk.ico" />
