...
These attacks, like all [NTLM relay attacks](/ad/movement/ntlm/relay), require a victim account to authenticate to an attacker-controlled machine. An attacker can coerce authentication by many means, see [MITM and coerced authentication coercion techniques](/ad/movement/mitm-and-coerced-authentications). Once the incoming authentication is received by the attacker, it can be relayed to an AD CS web endpoint.

Once the relayed session is obtained, the attacker poses as the relayed account and can request a client authentication certificate. The certificate template used needs to be configured for authentication (i.e. EKUs like Client Authentication, PKINIT Client Authentication, Smart Card Logon, Any Purpose (`OID 2.5.29.37.0`), or no EKU (`SubCA`)). The attacker obtains a certificate **as the relayed account** and authenticates as it, so the impact is scoped to whatever victim was coerced/relayed (typically a computer account such as a domain controller or Exchange server), not arbitrary user impersonation.
...
[Certipy](https://github.com/ly4k/Certipy) (Python) can be used to enumerate information regarding the certificate templates (EKUs allowing for authentication, allowing low-priv users to enroll, etc.) and identify enabled HTTP endpoint ([how to enumerate](/ad/movement/ad-cs#attack-paths)).
...
Just like any other NTLM relay attack, once the relay servers are running and waiting for incoming NTLM authentications, authentication coercion techniques can be used (e.g. [PrinterBug](/ad/movement/mitm-and-coerced-authentications/ms-rprn), [PetitPotam](/ad/movement/mitm-and-coerced-authentications/ms-efsr), [PrivExchange](/ad/movement/exchange-services/privexchange)) to force accounts/machines to authenticate to the relay servers.

<Callout type="info">

Read the [mitm-and-coerced-authentications](/ad/movement/mitm-and-coerced-authentications) article for more insight.
...
Once incoming NTLM authentications are relayed and authenticated sessions abused, base64-encoded PFX certificates will be obtained and usable with [Pass-the-Certificate](/ad/movement/kerberos/pass-the/pass-the-certificate) to obtain a TGT and authenticate.
...
[Certipy](https://github.com/ly4k/Certipy) (Python) can be used to enumerate information regarding the certificate templates (EKUs allowing for authentication, allowing low-priv users to enroll, etc.) and identify a vulnerable RPC endpoint ([how to enumerate](/ad/movement/ad-cs#attack-paths)).
...
<ResourceCard href="https://www.exandroid.dev/2021/06/23/ad-cs-relay-attack-practical-guide" title="AD CS relay attack - practical guide" description="www.exandroid.dev" icon="/assets/favicons/www.exandroid.dev.png" />

<ResourceCard href="https://dirkjanm.io/ntlm-relaying-to-ad-certificate-services" title="NTLM relaying to AD CS - On certificates, printers and a little hippo" description="dirkjanm.io" />
