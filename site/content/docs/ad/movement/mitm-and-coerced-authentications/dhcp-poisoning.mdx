...
* The `-w/--wpad` option to start the WPAD rogue server so that fake `wpad.dat` file can be served to requesting clients (i.e. [WPAD spoofing](/ad/movement/mitm-and-coerced-authentications/wpad-spoofing))
...
* forced and [captured](/ad/movement/ntlm/capture) with Responder with the command line above (with `--wredir` and `--ProxyAuth`)
* or forced and [relayed](/ad/movement/ntlm/relay) with [ntlmrelayx](https://github.com/SecureAuthCorp/impacket/blob/master/examples/ntlmrelayx.py) (by using the `--http-port 3128` argument
...
<ResourceCard href="https://g-laurent.blogspot.com/2021/08/responders-dhcp-poisoner.html" title="Laurent Gaffi\u00e9 blog: Responder's DHCP Poisoner" description="g-laurent.blogspot.com" icon="/assets/favicons/g-laurent.blogspot.com.ico" />
