...
1. [NTLM capture](/ad/movement/ntlm/capture) and [NTLM relay](/ad/movement/ntlm/relay) : spoof an SMB server and reroute received SMB packets to internal capture or relay servers **(rerouting technique)**.
2. [DNS spoofing](/ad/movement/mitm-and-coerced-authentications/dns-spoofing) : spoof an internal DNS server, so that DNS queries can be answered with fake resolutions **(rerouting technique)**.
...
* make sure the iptables rules are ok and allow forwarding --> [networking](/ad/movement/mitm-and-coerced-authentications/arp-poisoning#network-filter)
...
Below are examples or targetted ARP poisoning attacks where the attacker wants to hijack packets aimed at a specific server (SMB, DNS, WSUS, ...), to answer with evil responses. The "dumping network secrets" scenario is the one attackers use to [dump credentials on the network](/redteam/credentials/unsecured-credentials/network-protocols) (usually in order to find an initial foothold).

<Tabs items={['SMB spoofing', 'DNS spoofing', 'WSUS spoofing', 'Dumping network secrets']}>
<Tab value="SMB spoofing">

Start the SMB server for [capture](/ad/movement/ntlm/capture) or [relay](/ad/movement/ntlm/relay) then start the poisoning attack.
...
<Tab value="DNS spoofing">

Start the DNS server ([responder](https://github.com/lgandx/Responder), [dnschef](https://github.com/iphelix/dnschef), or [bettercap](https://github.com/bettercap/bettercap)) for [DNS poisoning](/ad/movement/mitm-and-coerced-authentications/dns-spoofing) then start the ARP poisoning attack.
...
<Tab value="Dumping network secrets">
...
<ResourceCard href="http://g-laurent.blogspot.com/2016/10/introducing-responder-multirelay-10.html" title="Laurent Gaffi\u00e9 blog: Introducing Responder MultiRelay 1.0" description="g-laurent.blogspot.com" icon="/assets/favicons/g-laurent.blogspot.com.png" />

<ResourceCard href="https://luemmelsec.github.io/Relaying-101/#arp-spoofing" title="LuemmelSec - Blog" description="luemmelsec.github.io" />

<ResourceCard href="https://www.bettercap.org/modules/ethernet/spoofers/arp.spoof/" title="www.bettercap.org" description="www.bettercap.org" />

<ResourceCard href="https://ivanitlearning.wordpress.com/2019/04/07/arp-dns-poisoning-with-bettercap-and-impacket-ntlmrelayx/" title="ARP & DNS Poisoning with Bettercap and Impacket NTLMRelayX" description="ivanitlearning.wordpress.com" icon="/assets/favicons/ivanitlearning.wordpress.com.png" />
