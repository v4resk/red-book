---
title: "Pass the Certificate - PKINIT"
---

## Theory

The Kerberos authentication protocol works with tickets in order to grant access. An ST (Service Ticket) can be obtained by presenting a TGT (Ticket Granting Ticket). That prior TGT can only be obtained by validating a first step named "pre-authentication" (except if that requirement is explicitly removed for some accounts, making them vulnerable to [ASREProast](/ad/movement/kerberos/roasting/asreproast)). The pre-authentication can be validated symmetrically (with a DES, RC4, AES128 or AES256 key) or asymmetrically (with certificates). The asymmetrical way of pre-authenticating is called PKINIT.

Pass the Certificate is the fancy name given to the pre-authentication operation relying on a certificate (i.e. key pair) to pass in order to obtain a TGT. This operation is often conducted along [shadow credentials](/ad/movement/kerberos/shadow-credentials), [AD CS escalation](/ad/movement/ad-cs) and [UnPAC-the-hash attacks](/ad/movement/kerberos/unpac-the-hash).
...
\
**However, you can try to** [**pass the certificate using Schannel**](/ad/movement/schannel/pass-the-certificate-schannel)**.**
...
* authenticate with [pass-the-cache](/ad/movement/kerberos/pass-the/ptc)
* conduct an [UnPAC-the-hash](/ad/movement/kerberos/unpac-the-hash) attack. This can be done with [getnthash.py](https://github.com/dirkjanm/PKINITtools/blob/master/getnthash.py) from [PKINITtools](https://github.com/dirkjanm/PKINITtools/).
...
When using Certipy for Pass-the-Certificate, it automatically does [UnPAC-the-hash](/ad/movement/kerberos/unpac-the-hash) to recover the account's NT hash, in addition to saving the obtained TGT.

```bash
certipy auth -pfx <PATH_TO_PFX_CERT> -password <CERT_PASSWORD> -dc-ip <DC_IP> -username <user> -domain <DOMAIN_FQDN>
```
...
[Evil-WinRM](https://github.com/Hackplayers/evil-winrm) uses the [WinRM](/redteam/pivoting/winrm) protocol based on [Windows Management Instrumentation (WMI)](/redteam/pivoting/remote-wmi) to give us an interactive shell on a Windows host. Winrm Supports PKINIT, so we can authenticate with a certificate.
...
* authenticate with [pass-the-ticket](/ad/movement/kerberos/pass-the/ptt)
* conduct an [UnPAC-the-hash](/ad/movement/kerberos/unpac-the-hash) attack (add the `/getcredentials` flag to Rubeus's asktgt command)
