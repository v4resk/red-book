...
There are ways to come across ([cached Kerberos tickets](/redteam/credentials/os-credentials/windows-and-active-directory/cached-kerberos-tickets)) or forge ([overpass the hash](/ad/movement/kerberos/pass-the/ptk), [silver ticket](/ad/movement/kerberos/forged-tickets) and [golden ticket](/ad/movement/kerberos/forged-tickets) attacks) Kerberos tickets. A ticket can then be used to authenticate to a system using Kerberos without knowing any password. This is called [Pass the ticket](/ad/movement/kerberos/pass-the/ptt). Another name for this is Pass the Cache (when using tickets from, or found on, UNIX-like systems).
