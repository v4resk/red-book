...
The TGT can then be used with [Pass the Ticket](/ad/movement/kerberos/ptt) for the next step, which can be conducted remotely if needed, unlike this initial step.
...
From UNIX-like systems, [Impacket](https://github.com/SecureAuthCorp/impacket)'s getST.py (Python) script can be used for the purpose. If needed, `.kirbi` files can be converted to `.ccache` (cf. [Pass the Ticket](/ad/movement/kerberos/ptt)).
...
Once a Service Ticket is received, it can be used with [pass-the-ticket](/ad/movement/kerberos/ptt)/[pass-the-cache](/ad/movement/kerberos/ptc) to obtain access to oneself as the "DomainAdmin" (the user can be changed in the request. Attackers should select a domain user which has local admin rights on the machine).
