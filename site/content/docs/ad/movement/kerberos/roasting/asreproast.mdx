...
AS-REP roasting can also be conducted **without** relying on pre-authentication being disabled, by holding a man-in-the-middle position and catching `AS-REP` responses. [ASRepCatcher](https://github.com/Yaxxine7/ASRepCatcher) (Python) does this, and can additionally force client workstations to downgrade to RC4 (weaker, faster to crack) by tampering with the Kerberos negotiation. It uses ARP spoofing natively, which can be disabled if the MitM is obtained by other means.

```bash
# Proxy between the clients and the DC, forcing an RC4 downgrade where supported
ASRepCatcher relay -dc $DC_IP

# Same, without the built-in ARP spoofing (MitM obtained another way)
...
Captured material reuses the usual `$krb5asrep$` format and cracks with hashcat `-m 18200`. See [MITM and coerced authentications](/ad/movement/mitm-and-coerced-authentications) for positioning.
