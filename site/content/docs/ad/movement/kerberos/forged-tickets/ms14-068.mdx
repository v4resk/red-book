...
Once the `.ccache` TGT is obtained, if the attack is successful, the ticket will be usable with [pass-the-ticket](/ad/movement/kerberos/pass-the/ptt). An easy way to check if the TGT works is to use it and ask for a service ticket. This can be done with Impacket's [getST.py](https://github.com/SecureAuthCorp/impacket/blob/master/examples/getST.py) (Python).
...
ms14068.exe /domain:$domain /user:$user /key:$USER_NThash /sid:$DomainSID /rid:$userRelativeID /kdc:$KDC_FQDN
```
