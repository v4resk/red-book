...
In order to craft a silver ticket, testers need to find the target service account's RC4 key (i.e. NT hash) or AES key (128 or 256 bits). This can be done by [capturing an NTLM response](/ad/movement/ntlm/capture) (preferably NTLMv1) and [cracking](/ad/movement/credentials/cracking) it, by [dumping LSA secrets](/redteam/credentials/os-credentials/windows-and-active-directory/sam-and-lsa-secrets), by doing a [DCSync](/redteam/credentials/os-credentials/windows-and-active-directory/dcsync), etc.
...
For both mimikatz and Rubeus, the `/ptt` flag is used to automatically [inject the ticket](/ad/movement/kerberos/pass-the/ptt#injecting-the-ticket).
...
A great, stealthier, alternative is to [abuse S4U2self](/ad/movement/kerberos/delegations/s4u2self-abuse) in order to impersonate a domain user with local admin privileges on the target machine by relying on Kerberos delegation instead of forging everything.

</Callout>

## Resources

<ResourceCard href="https://en.hackndo.com/kerberos-silver-golden-tickets/" title="Silver & Golden Tickets" description="en.hackndo.com" icon="/assets/favicons/en.hackndo.com.png" />
