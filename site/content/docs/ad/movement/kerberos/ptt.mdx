...
There are ways to come across ([cached Kerberos tickets](/redteam/credentials/os-credentials/windows-and-active-directory/cached-kerberos-tickets)) or forge ([overpass the hash](/ad/movement/kerberos/ptk), [silver ticket](/ad/movement/kerberos/forged-tickets#silver-ticket) and [golden ticket](/ad/movement/kerberos/forged-tickets#golden-ticket) attacks) Kerberos tickets. A ticket can then be used to authenticate to a system using Kerberos without knowing any password. This is called [Pass the ticket](/ad/movement/kerberos/ptt). Another name for this is Pass the Cache (when using tickets from, or found on, UNIX-like systems).
...
<Tabs items={['Credentials dumping', 'Command execution']}>
...
Their are multiple service classes for multiple service types (LDAP, CIFS, HTTP and so on) (more info on [adsecurity.org](https://adsecurity.org/?page\_id=183)). The problem here is that since the SPN is not protected, there are scenarios (e.g. services configured for [constrained delegations](/ad/movement/kerberos/delegations/constrained)) where the service class can be modified in the ticket, allowing attackers to have access to other types of services.
...
<ResourceCard href="https://www.secureauth.com/blog/kerberos-delegation-spns-and-more" title="www.secureauth.com" description="www.secureauth.com" icon="/assets/favicons/www.secureauth.com.png" />
