...
The ability to edit a machine account's `sAMAccountName` and `servicePrincipalName` attributes is a requirement to the attack chain. The easiest way this can be achieved is by creating a computer account (e.g. by leveraging the [MachineAccountQuota](/ad/movement/domain-settings/machineaccountquota) domain-level attribute if it's greater than 0). The creator of the new machine account has enough privileges to edit its attributes. Alternatively, taking control over the owner/creator of a computer account should do the job.
...
2. Change the controlled machine account `sAMAccountName` to a Domain Controller's name without the trailing `$` -> [CVE-2021-42278](/ad/movement/kerberos/principal-confusion/samaccountname-spoofing#cve-2021-42278---name-impersonation)
...
5. Request a service ticket with S4U2self by presenting the TGT obtained before -> [CVE-2021-42287](/ad/movement/kerberos/principal-confusion/samaccountname-spoofing)
6. Get access to the domain controller (i.e. [DCSync](/redteam/credentials/os-credentials/windows-and-active-directory/dcsync))

<Callout type="warn">

Impacket's `getST` (`-self` / `-altservice`) is merged and ships in current Impacket. The `renameMachine.py` helper is still an open pull request ([#1224](https://github.com/fortra/impacket/pull/1224)), so it must be run from that PR branch.
...
* with [Mimikatz](https://github.com/gentilkiwi/mimikatz) (C) for the [DCSync](/redteam/credentials/os-credentials/windows-and-active-directory/dcsync) operation
...
<Figure src="/assets/spaces_-MHRw3PMJtbDDjbxm5ub_uploads_JCIbFHflwCsQx8sO9d2J_Screenshot from 2021-12-10 22-21-10.webp" />
...
This attack path also requires knowledge of the user account password or hash (to obtain a TGT), which can be obtained (or set) in many ways (e.g. [Targeted Kerberoasting](/ad/movement/dacl/targeted-kerberoasting), [Shadow Credentials](/ad/movement/kerberos/shadow-credentials), [Forced Password Change](/ad/movement/dacl/forcechangepassword)).

Appart from the computer account creation and SPNs manipulation, the exploitation steps are the same as with a [machine account](/ad/movement/kerberos/principal-confusion/samaccountname-spoofing#machine-account). If the account has SPNs that point to its name, they will have to be removed for the renaming operation to work.

## Resources

<ResourceCard href="https://exploit.ph/cve-2021-42287-cve-2021-42278-weaponisation.html" title="eXploit - CVE-2021-42287/CVE-2021-42278 Weaponisation" description="exploit.ph" icon="/assets/favicons/exploit.ph.png" />

<ResourceCard href="https://exploit.ph/more-samaccountname-impersonation.html" title="eXploit - More sAMAccountName Impersonation" description="exploit.ph" icon="/assets/favicons/exploit.ph.png" />

<ResourceCard href="https://cloudbrothers.info/en/exploit-kerberos-samaccountname-spoofing" title="Exploit samAccountName spoofing with Kerberos" description="cloudbrothers.info" icon="/assets/favicons/cloudbrothers.info.png" />

<ResourceCard href="https://twitter.com/snovvcrash/status/1471829627765239816" title="sn\ud83e\udd76vvcr\ud83d\udca5sh (@snovvcrash) on X" description="twitter.com" icon="/assets/favicons/twitter.com.ico" />
