...
In standard KCD abuse scenarios, an attacker that gains control over a "KCD service" can operate lateral movement and obtain access to the other services/SPNs. Since KCD allows for impersonation, the attacker can also impersonate users (e.g. domain admins) on the target services. Depending on the SPNs, or if it's possible to [modify it](/ad/movement/kerberos/ptt#modifying-the-spn), the attacker could also gain admin access to the server the "listed SPN" belongs to.
...
Once the final service ticket is obtained, it can be used with [Pass the Cache](/ad/movement/kerberos/ptc) / [Pass the Ticket](/ad/movement/kerberos/ptt) to access the target.
...
Once the final service ticket is obtained, it can be used with [Pass the Ticket](/ad/movement/kerberos/ptt) to access the target.
