...
2. Change the controlled machine account `sAMAccountName` to a Domain Controller's name without the trailing `$` -> [CVE-2021-42278](/ad/movement/kerberos/samaccountname-spoofing#cve-2021-42278-name-impersonation)
...
5. Request a service ticket with S4U2self by presenting the TGT obtained before -> [CVE-2021-42287](/ad/movement/kerberos/samaccountname-spoofing#cve-2021-42287-kdc-lookup)
...
Appart from the computer account creation and SPNs manipulation, the exploitation steps are the same as with a [machine account](/ad/movement/kerberos/samaccountname-spoofing#machine-account). If the account has SPNs that point to its name, they will have to be removed for the renaming operation to work.
