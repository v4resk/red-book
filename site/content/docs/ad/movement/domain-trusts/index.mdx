...
**This is a work-in-progress**. It's indicated with the emoji in the page name or in the category name
...
![](/assets/SID_filtering_default_configs.BWeXqOoS.png)
...
![](/assets/SID_filtering_custom_configs.CB0x9lVQ.png)
...
* there are two kinds of inter-forest trusts: "Forest", and "External" (see [trust types](/ad/movement/domain-trusts#trust-types)). Microsoft says "[cross-forest trusts are more stringently filtered than external trusts](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-dtyp/81d92bba-d22b-4a8c-908a-554ab29148ab?redirectedfrom=MSDN)", meaning that in External trusts, SID filtering only filters out RID &lt; 1000.

![](/assets/MS_PAC_section_4.1.2.2.Cmw5BXcP.png)
...
Also, SID filtering works the same way for NTLM and Kerberos. It's a separate mechanism invoked after user logon info are unpacked (more details in [NTLM](/ad/movement/domain-trusts#ntlm-authentication) and [Kerberos](/ad/movement/domain-trusts#kerberos-authentication) chapters).
...
Kerberos unconstrained delegation (KUD) allows a service configured for it to impersonate (almost) any user on any other service. This is a dangerous feature to configure, that won't be explained into many details here as the [Kerberos](/ad/movement/kerberos#delegations), [Kerberos delegations](/ad/movement/kerberos/delegations) and [Kerberos unconstrained delegations](/ad/movement/kerberos/delegations/unconstrained) pages already cover it.
...
Understanding how Kerberos works is required here: [the Kerberos protocol](/ad/movement/kerberos).
...
From an offensive point of view, just like a [golden ticket](/ad/movement/kerberos/forged-tickets/golden), a referral ticket could be forged. Forging a referral ticket using the inter-realm key, instead of relying on the krbtgt keys for a golden ticket, is a nice alternative for organizations that choose to roll their krbtgt keys, as they should. This technique is [a little bit trickier](https://dirkjanm.io/active-directory-forest-trusts-part-two-trust-transitivity/#do-you-need-to-use-inter-realm-tickets) though, as it requires to [use the correct key](https://dirkjanm.io/active-directory-forest-trusts-part-two-trust-transitivity/#which-keys-do-i-need-for-inter-realm-tickets).

Depending on the trust characteristics, ticket forgery can also be combined with [SID history](/ad/movement/domain-trusts#sid-history) spoofing for a direct privilege escalation from a child to a parent domain.

When doing Kerberos authentications across trusts, the trusting domain's domain controller [checks a few things](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-kile/bac4dc69-352d-416c-a9f4-730b81ababb3) before handing out service tickets to trusted users: [SID filtering](/ad/movement/domain-trusts#sid-filtering) during [PAC validation](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-pac/55fc19f2-55ba-4251-8a6a-103dd7c66280) (looking in the `ExtraSids` attribute from the [`KERB_VALIDATION_INFO`](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-pac/69e86ccc-85e3-41b9-b514-7d969cd0ed73) structure in the PAC), [TGT delegation](/ad/movement/domain-trusts#tgt-delegation) verification (when asked for a Service Ticket for a service configured for unconstrained delegation), and [Selective Authentication](/ad/movement/domain-trusts#authentication-level) limitation.
...
When doing NTLM authentications across trusts, the trusting domain's domain controller checks a few things from the user info structure supplied by the trusted domain controller: [SID filtering](/ad/movement/domain-trusts#sid-filtering) (looking in the `ExtraSids` attribute from the [`NETLOGON_VALIDATION_SAM_INFO2`](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-nrpc/2a12e289-7904-4ecb-9d83-6732200230c0) structure), and [Selective Authentication](/ad/movement/domain-trusts#authentication-level) limitation during the [DC's validation of the user credentials](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-apds/f47e40e1-b9ca-47e2-b139-15a1e96b0e72). [TGT delegation](/ad/movement/domain-trusts#tgt-delegation) verification doesn't occur here, since it's a Kerberos mechanism.
...
Microsoft introduced MIM (Microsoft Identity Manager) Privileged Access Management (PAM) with Server 2016. Sometimes referred to as PIM, it brings:

* a **Bastion forest** (a forest in ESAE, Enhanced Security Admin Environment, also called a Red Forest)
* **Shadow Security Principals**, letting admins in the Bastion forest be mapped as Domain Admins of a production forest
* **temporary group membership**, adding a user to a group with a time-to-live

PAM manages an existing production forest from a Bastion forest holding a one-way PAM trust with it. Users in the Bastion forest are mapped to privileged groups such as Domain Admins or Enterprise Admins in the production forest **without modifying any group membership or ACL**, which removes administrative overhead and reduces lateral movement opportunities.

<Callout type="info">
This section is a light adaptation of [Nikhil Mittal's](https://www.labofapenetrationtester.com/2019/04/abusing-PAM.html) and [Daniel Ulrichs's](https://secureidentity.se/msds-shadowprincipal/) work.
</Callout>
...
[BloodHound](/ad/recon/tools/bloodhound) can also be used to map the trusts. While it doesn't provide much details, it shows a visual representation.
...
Beyond the techniques above, the following are known trust attack paths that are
not yet covered here. They are listed so the surface is visible even where the
detail is missing:

* **Keys container trust** and **DNS trust** abuse
* **GPO on site** applied across a trust
* **GoldenGMSA** across a forest boundary
* **Schema change attack** from child to parent ([improsec](https://improsec.com/tech-blog/sid-filter-as-security-boundary-between-domains-part-6-schema-change-trust-attack-from-child-to-parent))
* **DACL abuse**, **AD-CS abuse** and **group memberships** spanning a trust
* **Unconstrained delegation** abuse across a trust

## Resources

<ResourceCard href="https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2003/cc759554(v=ws.10)?redirectedfrom=MSDN" title="Trust Technologies: Domain and Forest Trusts" description="docs.microsoft.com" icon="/assets/favicons/docs.microsoft.com.png" />

<ResourceCard href="https://www.thehacker.recipes/ad/movement/trusts" title="Trusts | The Hacker Recipes" description="www.thehacker.recipes" icon="/assets/favicons/www.thehacker.recipes.png" />

<ResourceCard href="https://mayfly277.github.io/posts/GOADv2-pwning-part12/" title="GOAD - part 12 - Trusts" description="mayfly277.github.io" icon="/assets/favicons/mayfly277.github.io.png" />

<ResourceCard href="https://medium.com/r3d-buck3t/breaking-domain-trusts-with-forged-trust-tickets-5f03fb71cd72" title="medium.com" description="medium.com" icon="/assets/favicons/medium.com.ico" />

<ResourceCard href="https://blog.improsec.com/tech-blog/sid-filter-as-security-boundary-between-domains-part-7-trust-account-attack-from-trusting-to-trusted" title="SID filter as security boundary between domains? (Part 7) - Trust account attack - from trusting to trusted" description="blog.improsec.com" icon="/assets/favicons/blog.improsec.com.png" />

<ResourceCard href="https://attack.mitre.org/techniques/T1482/" title="Domain Trust Discovery, Technique T1482 - Enterprise | MITRE ATT&CK" description="attack.mitre.org" icon="/assets/favicons/attack.mitre.org.png" />

<ResourceCard href="https://ppn.snovvcrash.rocks/pentest/infrastructure/ad/attack-trusts" title="ppn.snovvcrash.rocks" description="ppn.snovvcrash.rocks" />

<ResourceCard href="http://www.harmj0y.net/blog/redteaming/a-guide-to-attacking-domain-trusts/" title="www.harmj0y.net" description="www.harmj0y.net" icon="/assets/favicons/www.harmj0y.net.png" />

<ResourceCard href="https://www.semperis.com/blog/ad-security-research-breaking-trust-transitivity/" title="Transitive Trust and Active Directory: Semperis AD 101" description="www.semperis.com" icon="/assets/favicons/www.semperis.com.png" />
