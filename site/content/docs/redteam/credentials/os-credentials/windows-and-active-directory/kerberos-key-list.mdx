...
An attacker can abuse this by forging a [RODC golden ticket](/ad/movement/kerberos/forged-tickets/rodc-golden-tickets) for a target user and use it to send a `TGS-REQ` to the `KRBTGT` service with a `padata` filed value of [161](https://learn.microsoft.com/en-us/openspecs/windows\_protocols/ms-kile/ae60c948-fda8-45c2-b1d1-a71b484dd1f7) (`KERB-KEY-LIST-REQ`). Knowing the `KRBTGT` key of the RODC is required here. The `TGS-REP` will contain the long term secret of the user in the `KERB-KEY-LIST-REP` key value.
...
<ResourceCard href="https://www.thehacker.recipes/ad/movement/credentials/dumping/kerberos-key-list" title="Kerberos key list | The Hacker Recipes" description="www.thehacker.recipes" icon="/assets/favicons/www.thehacker.recipes.png" />
