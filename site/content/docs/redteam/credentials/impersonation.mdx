...
On windows or Active Directory environments, when credentials are found (through [dumping](/ad/movement/credentials/dumping) or [cracking](/ad/movement/credentials/cracking) for instance), attackers try to use them to obtain access to new resources. Depending on the harvested credential material type, the impersonation can be done in different ways.

* **LM or NT password hash**: [pass-the-hash](/ad/movement/ntlm/pth)
* **RC4 Kerberos key (i.e. NT hash)**: [overpass-the-hash](/ad/movement/kerberos/pass-the/ptk)
* **non-RC4 Kerberos key (i.e. DES or AES)**: [pass-the-key](/ad/movement/kerberos/pass-the/ptk) (alias for overpass-the-hash)
* **Kerberos ticket**: [pass-the-ticket](/ad/movement/kerberos/pass-the/ptt)
* **plaintext password**: the techniques listed below

<Tabs items={['RunAs', 'RunAsCs', 'Powershell', 'PowerView']}>
<Tab value="RunAs">
...
Here is an example for [targeted Kerberoasting](/ad/movement/dacl/targeted-kerberoasting).
